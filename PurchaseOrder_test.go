...
var (
	pos =[]*PurchaseOrder{ NewPurchaseOrder("PO1")}
)
...
func TestUnmarshalPurchaseOrder(t *testing.T ){
	...
	if len(json_example) != 1 || json_example[0].FromCoCo!="CO1" || json_example[0].ToCoCo!="CO2" || len(json_example[0].LineLevel) != 1 {
		fmt.Printf("failed to Unmarshal example file. ")
		t.Fail()
	}
}
